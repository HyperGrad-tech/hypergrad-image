...
export default function ColorExtract() {
  ...
  const extract = useCallback((img: HTMLImageElement) => {
    // Limit sampling size to avoid memory issues with large images
    const maxDim = 200;
    ...
    // Filter fully transparent pixels
    const opaque: number[] = [];
    ...
    <div>
      <div class="tool-card">
        ...
        {!imgSrc ? (
          <div onClick={() => inputRef.current?.click()} style={{ border: '2px dashed var(--border-strong)', borderRadius: 'var(--radius-md)', padding: '60px', textAlign: 'center', cursor: 'pointer', color: 'var(--text-muted)' }}>
            ...
            <div class="text-sm">Extract dominant colors to build a palette, processed locally with no upload</div>
          </div>
        ) : (
          <div class="toolbar">
            <button class="btn btn-secondary btn-sm" onClick={() => inputRef.current?.click()}>Replace image</button>
            <div>
              <label class="text-xs text-muted">Color count</label>
              <select class="select" value={count} onChange={e => setCount(Number(e.target.value))} style={{ marginLeft: '8px' }}>
                {[3, 4, 5, 6, 8, 10, 12].map(n => <option value={n}>{n} colors</option>)}
              </select>
            </div>
            <button class="btn btn-primary btn-sm" onClick={reExtract}>Re-extract</button>
          </div>
        )}
      </div>
      ...
      {imgSrc && (
        <div class="tool-grid-2">
          <div class="tool-card">
            <div class="font-bold mb-md">Original Image</div>
            <img src={imgSrc} alt="Original" style={{ maxWidth: '100%', border: '1px solid var(--border)', borderRadius: 'var(--radius)' }} />
          </div>
          <div class="tool-card">
            <div class="font-bold mb-md">Extracted Palette</div>
            {colors.length === 0 ? (
              <div class="text-muted">Extracting...</div>
            ) : (
                ...
                {colors.map((c, i) => (
                  ...
                  >
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div class="text-xs text-muted">{copied === c.hex ? 'Copied!' : 'Click to copy'}</div>
                    </div>
                  </div>
                ))}
              </div>
