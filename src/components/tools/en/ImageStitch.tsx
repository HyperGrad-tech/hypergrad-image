...
export default function ImageStitch() {
  ...
  const handleFiles = useCallback((fileList: FileList) => {
    ...
    Promise.all(arr.map(f => new Promise<LoadedImg>((resolve, reject) => {
      ...
      img.onerror = () => reject(new Error(f.name + ' load failed'));
      img.src = url;
    ...
    <div>
      <div class="tool-card">
        ...
        <div onClick={() => inputRef.current?.click()} style={{ border: '2px dashed var(--border-strong)', borderRadius: 'var(--radius-md)', padding: '40px', textAlign: 'center', cursor: 'pointer', color: 'var(--text-muted)' }}>
          ...
          <div class="font-bold" style={{ color: 'var(--text)', marginBottom: '4px' }}>Click to add images (multi-select supported)</div>
          <div class="text-sm">Stitched locally, no upload</div>
        </div>
      </div>
      ...
      {imgs.length > 0 && (
        <>
          <div class="tool-card">
            <div class="font-bold mb-md">Stitch Settings</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: '12px' }}>
              <div>
                <label class="text-xs text-muted">Direction</label>
              ...
              <div>
                <label class="text-xs text-muted">Alignment</label>
              ...
              <div>
                <label class="text-xs text-muted">Uniform size</label>
                <select class="select w-full mt-md" value={resizeMode} onChange={e => setResizeMode(e.target.value as ResizeMode)}>
                  ...
                  <option value="unify">Scale to uniform size</option>
                </select>
              </div>
              <div>
                <label class="text-xs text-muted">Gap (px)</label>
          ...
          <div class="tool-card">
            <div class="toolbar">
              <div class="font-bold">{imgs.length} images loaded</div>
              <button class="btn btn-secondary btn-sm" onClick={() => inputRef.current?.click()}>Add more</button>
            </div>
              ...
              {imgs.map((item, i) => (
                <div key={item.id} style={{ position: 'relative', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '4px' }}>
                  ...
                  <div style={{ position: 'absolute', top: '-8px', right: '-8px', display: 'flex', gap: '2px' }}>
                    <button class="btn btn-secondary btn-sm" style={{ minWidth: '24px', padding: '0', height: '22px' }} onClick={() => move(i, -1)} disabled={i === 0} title="Move up">↑</button>
                    <button class="btn btn-secondary btn-sm" style={{ minWidth: '24px', padding: '0', height: '22px' }} onClick={() => move(i, 1)} disabled={i === imgs.length - 1} title="Move down">↓</button>
                    <button class="btn btn-danger btn-sm" style={{ minWidth: '24px', padding: '0', height: '22px' }} onClick={() => removeAt(i)} title="Remove">×</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          {outUrl && (
            <div class="tool-card">
              <div class="toolbar">
                <div class="font-bold">Result · {outDim.w}×{outDim.h}</div>
                <button class="btn btn-primary btn-sm" onClick={download}>Download PNG</button>
              </div>
              <div style={{ border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '16px', textAlign: 'center', background: 'var(--bg-soft)', marginTop: '12px' }}>
                <img src={outUrl} alt="Stitched result" style={{ maxWidth: '100%', maxHeight: '500px', objectFit: 'contain' }} />
              </div>
            </div>
          )}
        </>
